...
function Human_x:OnEnemySeen()
	Log("In OnEnemySeen Function")
	
	AIBase.OnEnemySeen(self);
	
	local attentionTarget = AI.GetAttentionTargetEntity(self.id);

	if (attentionTarget.Properties.esFaction == "Snake" ) then
		Log("Enemy = Snake")
		--AI.Signal(SIGNALFILTER_SENDER, 1, "OnEnemySeen", self.id);
	end
	...
	--[[
	if (attentionTarget.Properties.esFaction == "Food" and attentionTarget.actor:GetHealth() < 500) then
		AI.Signal(SIGNALFILTER_SENDER, 1, "OnFoodSeen", self.id);
	end
	]]
	
end

function Human_x:Search()
	Log("In Search Function")
	
end

function Human_x:Sleep()
	Log("In Sleep Function")
	
end

function Human_x:Avoid()
	Log("In Avoid Function")
